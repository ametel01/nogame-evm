...
contract Facilities {
    ...
    function _dockyardRequirementsCheck(
        address caller
    ) private returns (bool) {}

    function _microtechRequirementsCheck(
        address caller
    ) private returns (bool) {}

    function _automationUpgradeCost(
        uint currentLevel
    ) private returns (uint metal, uint crystal, uint deuterium) {}

    function _dockyardUpgradeCost(
        uint currentLevel
    ) private returns (uint metal, uint crystal, uint deuterium) {}

    function _scienceLabUpgradeCost(
        uint currentLevel
    ) private returns (uint metal, uint crystal, uint deuterium) {}

    function _microtechUpgradeCost(
        uint currentLevel
    ) private returns (uint metal, uint crystal, uint deuterium) {}
}
